...
export function TopBar() {
  ...
  const { guests, relationships, tables, resetChart, undo, _history } = useStore()
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full mx-4 p-6 flex flex-col gap-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3 text-sm text-stone-600">
              ...
              <Section title="Undo / Import / Export">
                <Kbd>Undo</Kbd> reverts the last action (also <Kbd>⌘Z</Kbd>). <Kbd>Export</Kbd> saves your chart as JSON. <Kbd>+ Add Multiple Guests</Kbd> in the guest panel loads a saved chart or a CSV/TXT guest list. Your chart auto-saves to your browser.
              </Section>
            </div>
          </div>
        </div>
